...
d3Chart.create = function(node, config, state){
	var svg = d3.select(node).append('svg')
		.attr('class', 'd3')
		.attr('width', config.width+config.axis_margins.left+150)//150 is for the legend
		.attr('height', config.height+config.axis_margins.bottom);


	svg.append('g')
		.attr('class', 'd3-bars'); //g element is a container
...
d3Chart._drawAxes = function(node, config, scales){
	...
	var xAxis = d3.svg.axis()
		.scale(scales.xscale)
		.orient("bottom")
		.tickValues(d3.range(x_min, x_max+1, x_step))
		.tickFormat(d3.format("d")); //Removes commas from axis labels
	
	d3.select(node).selectAll('.d3').append("text")
	    .attr("class", "axis-label")
	    .attr("x", (config.width+config.axis_margins.left)/2)
	    .attr("y", (config.height+(config.axis_margins.bottom/2))
	    .text(config.x_label);

	var yAxis = d3.svg.axis()
		.scale(scales.yscale)
		.orient("right")
		.ticks(scales.yscale.domain()[1])
	...
	xg.attr("transform", "translate("+config.axis_margins.left+", "+config.height+")")
	  .call(xAxis)
	  .selectAll("text")
	...
	yg.attr("transform", "translate("+config.axis_margins.left+", 0)")
	  .call(yAxis);
...
d3Chart._drawGridlines = function(node, config, scales){
	//I want to show all the tick values by default
	var x_step = 1;
	var x_domain = scales.xscale.domain();
	var x_min = x_domain[0];
...
d3Chart._drawBars = function(node, config, scales, data, dispatcher){
	...
	bar.enter().append('rect')
		.attr('class', function(d){ return 'd3-bar d3-zcolor-'+d.z});

	//Update
	bar.attr('x', function(d){ return config.axis_margins.left+scales.xscale(d.x)+(barwidth*scales.zscale(d.z))})
	   .attr('y', function(d) { return scales.yscale(d.y)})
       .attr('width', barwidth)
       .attr('height', function(d) { return scales.height - scales.yscale(d.y) })
       .on('click', function(d) {
       		var this_bar = d3.select(this);
       		var myclass = this_bar.attr('class');
       		var highlight = "-highlight";
       		if(myclass.indexOf(highlight) > -1){ //if it is highlighted
...
d3Chart._drawLegend = function(node, config, zmap){
	var legend = d3.select(node).selectAll(".legend")
	  .attr("transform", function(d) { return "translate("+(config.width+config.axis_margins.left)+", 150)"; })
    ...
    label.enter().append("text")
          .attr("class", "label")
	      .attr("x", 24)
          .attr("y", function(d) { return (9+(30*d.value))})
          .attr("dy", ".35em")
          .style("text-anchor", "end")
          .text(function(d) { return d.text; });

    label.enter().append("rect")
	      .attr("class", function(d) { return 'label d3-zcolor-'+d.value})
	      .attr("x", 50)
	      .attr("y", function(d) { return ((30*d.value))})
	      .attr("width", 18)
	      .attr("height", 18);
...
d3Chart._scales = function(node, config, domain) {
  ...
  var x = d3.scale.linear()
    .range([0, width])
    .domain(shifted_xdomain);

  var y = d3.scale.linear()
    .range([height, 20]) //To avoid cutting off the top of the y axis
